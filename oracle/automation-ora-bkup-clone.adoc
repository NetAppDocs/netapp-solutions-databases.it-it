...
* Un proprietario di un'applicazione che desidera clonare i database Oracle da una copia di backup dell'immagine del database per altri casi d'uso.
...
NOTE: Per semplificare, il server Oracle di destinazione del clone dovrebbe essere configurato in modo identico al server Oracle primario, come lo stack software Oracle e il layout delle directory per Oracle Home, ecc.
...
[%collapsible%open]
====
Il server Oracle di destinazione clone deve avere lo stesso stack software Oracle del server Oracle di origine installato e patchato.  Idealmente, la variabile $ORACLE_HOME dovrebbe corrispondere all'impostazione del server Oracle di origine.  Se l'impostazione ORACLE_HOME di destinazione è diversa dalla configurazione del server Oracle primario, creare un collegamento simbolico per aggirare le differenze.

Se il database primario è configurato con ASM, il gruppo primario dei file di dati può appartenere al gruppo asm e lo stesso gruppo asm con lo stesso ID gruppo deve essere aggiunto all'host clone per evitare problemi di autorizzazione.
...
Per clonare eventuali database aggiuntivi, creare un file oracle_bkup_clone_n_gcnv.yml e un file oracle_bkup_clone_n_gcnv.sh separati.  Configurare di conseguenza gli host di destinazione Ansible, il file vars.yml globale e il file hostname.yml nella directory host_vars.
...
=== Dettagli di esecuzione del playbook

[%collapsible%open]
====
Il seguente file di registro cattura i dettagli dell'output di un'esecuzione completa del playbook a scopo di riferimento.
